...
Handle data preprocessing / augmentations.

Does NOT handle data file prep. See util.py for daa file prep and
...
class RadDataset(Dataset):
    def __init__(self, dataframe: torch.Tensor, partial_path: str, transform=None):
        ...
        Args:
            dataframe (torch.Tensor): A dataframe containing the paths to the
    ...
    def __getitem__(self, idx: int):
        """Fetches the image and label at the index `idx` and applies
        ...
        Returns:
            tuple: (image, label) where image is the transformed image, and
                label is the corresponding binary label.
        """
...
class CaffeTransform:
    ...
    ):  # set mean to imagenet caffe style means
        self.mean = torch.tensor(mean).view(3, 1, 1)  # convert to tensor
        self.mean = (
            self.mean * 1.0 / 255
        )  # rescale to apply to Tensor img in range [0,1]
...
def create_dataloaders(
    ...
    Returns:
        tuple: (train_loader, val_loader) DataLoaders for the training and
            validation datasets.
    """
    train_transform = transforms.Compose(
        [
            ...
            transforms.RandomRotation(
                10, interpolation=InterpolationMode.NEAREST, fill=128
            ),
            transforms.RandomAffine(
                degrees=0,
                translate=(0.1, 0.1),  # matches
                scale=(0.9, 1.1),  # matches
                shear=0.1,  # matches https://github.com/keras-team/keras/blob/v3.1.0/keras
                # /legacy/preprocessing/image.py
            ),
            transforms.ToTensor(),
    ...
    train_dataset = RadDataset(
        train_df, transform=train_transform, partial_path=partial_path
    )
